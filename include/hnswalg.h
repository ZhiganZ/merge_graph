...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    std::vector<int> element_inbound_;  
    std::vector<std::mutex> inbound_locks_;
    ...
    HierarchicalNSW(
        ...
        : label_op_locks_(MAX_LABEL_OPERATION_LOCKS),
            ...
            element_inbound_(max_elements),
            inbound_locks_(max_elements),
            allow_replace_deleted_(allow_replace_deleted) {
        max_elements_ = max_elements;
        for(int i = 0;i<max_elements;i++){
            element_inbound_[i]=0;
            element_levels_[i]=-1;
        }
        num_deleted_ = 0;
    ...
    searchBaseLayer_with_visited_node(
        ...
        if (bare_bone_search || 
            (!isMarkedDeleted(ep_id) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id))))) {
            char* ep_data = getDataByInternalId(ep_id);
            dist_t dist = fast_dist(data_point, ep_data, dist_func_param_,ep_id);
            if(!isMarkedDeleted(ep_id))
        ...
        while (!candidate_set.empty()) {
            ...
            for (size_t j = 1; j <= size; j++) {
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    ...
                    char *currObj1 = (getDataByInternalId(candidate_id));
                    dist_t dist = fast_dist(data_point, currObj1, dist_func_param_, candidate_id);
    ...
    dist_t fast_dist(const void *pVect1v, const void *pVect2v, const void *qty_ptr, const tableint dist) const {
        if (collect_metrics) 
            metric_distance_computations++;
        return fstdistfunc_(pVect1v, pVect2v, qty_ptr);
    }
...

        {
            ...
            for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
                ...
                if (level > element_levels_[selectedNeighbors[idx]]){
                    std::cout<<"mutuallyConnectNewElement error"<<std::endl;
                    std::cout<<idx<<" "<<element_levels_[selectedNeighbors[idx]]<<" "<<selectedNeighbors[idx]<<" "<<level<<std::endl;
                    std::cout<<cur_c<<std::endl;
                    continue;
                    throw std::runtime_error("Trying to make a link on a non-existent level_");
                }
    ...
    void get_one_hop_neigh(tableint node,int & layer, std::unordered_set<tableint> & two_hop_neighs) {
        auto one_hop_neis = getConnectionsWithLock(node, layer);
        for(auto && one_hop_neigh:one_hop_neis){
            if(!isMarkedDeleted(one_hop_neigh))two_hop_neighs.insert(one_hop_neigh);
        }
    }
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        tableint currObj = enterpoint_node_;
        dist_t curdist = fast_dist(query_data, getDataByInternalId(enterpoint_node_), dist_func_param_,enterpoint_node_);
        ...
        for (int level = maxlevel_; level > 0; level--) {
            ...
            while (changed) {
                ...
                for (int i = 0; i < size; i++) {
                    tableint cand = datal[i];
                    dist_t d = fast_dist(query_data, getDataByInternalId(cand), dist_func_param_,cand);
    ...
    uint64_t quick_check() {
        ...
        std::cout<<"最大入度为:"<<*std::max_element(inbound.begin(),inbound.end())<<std::endl;
        std::cout<<"最大入度的标签为:"<<std::max_element(inbound.begin(),inbound.end())-inbound.begin()<<std::endl;
        std::cout<<"check "<<total_inbound<<" connections"<<std::endl;
        std::cout<<"0入度点个数是"<<zero_inbound_count<<std::endl;
    ...
    std::vector<tableint> range_get(){
        std::vector<tableint> ret(max_elements_);
        for(int i = 0 ; i < max_elements_; ++i){
            ret[getExternalLabel(i)] = i;
        }
        return ret;
    }
