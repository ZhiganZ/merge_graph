...
namespace my_tool {
    ...
    void copy_meta_data_from_multiple(mergegraph::HierarchicalNSW<float>* alg_hnsw_merge, 
        ...
        for (const auto& graph : source_graphs) {
            graph->setEf(alg_hnsw_merge->ef_construction_);
        }

        if (total_elements > alg_hnsw_merge->max_elements_) {
            throw std::runtime_error("The merged graph size exceeds the max_elements capacity of the destination graph.");
        }

        alg_hnsw_merge->cur_element_count = total_elements;
        std::cout<<"element number after merge "<<alg_hnsw_merge->cur_element_count<<std::endl;
        alg_hnsw_merge->maxlevel_ = max_level;
        alg_hnsw_merge->enterpoint_node_ = source_graphs[0]->enterpoint_node_; 
        ...
        for (const auto& source_graph : source_graphs) {
            ...
            for (size_t i = 0; i < source_size; ++i) {
                ...
                memcpy(alg_hnsw_merge->getExternalLabeLp(new_id), source_graph->getExternalLabeLp(i), sizeof(mergegraph::labeltype));

                alg_hnsw_merge->element_levels_[new_id] = source_graph->element_levels_[i];

                memcpy(alg_hnsw_merge->getDataByInternalId(new_id), source_graph->getDataByInternalId(i), alg_hnsw_merge->data_size_);
            }

            for (size_t i = 0; i < source_size; ++i) {
                size_t new_id = i + cumulative_offset;
                alg_hnsw_merge->label_lookup_[source_graph->getExternalLabel(i)] = new_id;
            }
        ...
        for (const auto& source_graph : source_graphs) {
            ...
            for (size_t old_id = 0; old_id < source_size; ++old_id) {
                ...
                mergegraph::linklistsizeint* dest_ll_0 = alg_hnsw_merge->get_linklist0(new_id);
                alg_hnsw_merge->setListCount(dest_ll_0, neighbor_count_0);
                ...
                int element_level = alg_hnsw_merge->element_levels_[new_id]; 
                for (int level = 1; level <= element_level; ++level) {
                    ...
                    mergegraph::linklistsizeint* dest_ll = alg_hnsw_merge->get_linklist(new_id, level);
                    alg_hnsw_merge->setListCount(dest_ll, neighbor_count);
    ...
    void copy_meta_data(mergegraph::HierarchicalNSW<float>* alg_hnsw_merge,mergegraph::HierarchicalNSW<float>* first_half,mergegraph::HierarchicalNSW<float>* second_half){
        ...
        first_half->setEf(alg_hnsw_merge->ef_construction_);
        second_half->setEf(alg_hnsw_merge->ef_construction_);
        ...
        alg_hnsw_merge->cur_element_count = total_elements;
        alg_hnsw_merge->maxlevel_ = std::max(first_half->maxlevel_, second_half->maxlevel_);
        alg_hnsw_merge->enterpoint_node_ = first_half->enterpoint_node_; 
        ...
        for (size_t i = 0; i < first_half_size; ++i) {
            ...
            memcpy(alg_hnsw_merge->getExternalLabeLp(i), first_half->getExternalLabeLp(i), sizeof(mergegraph::labeltype));

            alg_hnsw_merge->element_levels_[i] = first_half->element_levels_[i];

            memcpy(alg_hnsw_merge->getDataByInternalId(i), first_half->getDataByInternalId(i), alg_hnsw_merge->data_size_);
        }

        for (size_t i = 0; i < first_half_size; ++i) {
            alg_hnsw_merge->label_lookup_[first_half->getExternalLabel(i)] = i;
        }
        ...
        for (size_t i = 0; i < second_half_size; ++i) {
            ...
            memcpy(alg_hnsw_merge->getExternalLabeLp(new_id), second_half->getExternalLabeLp(i), sizeof(mergegraph::labeltype));

            alg_hnsw_merge->element_levels_[new_id] = second_half->element_levels_[i];

            memcpy(alg_hnsw_merge->getDataByInternalId(new_id), second_half->getDataByInternalId(i), alg_hnsw_merge->data_size_);
        }
        for (size_t i = 0; i < second_half_size; ++i) {
            size_t new_id = i + first_half_size; 

            alg_hnsw_merge->label_lookup_[second_half->getExternalLabel(i)] = new_id;
        }


        #pragma omp parallel for
        for (size_t i = 0; i < total_elements; ++i) {
            if (alg_hnsw_merge->element_levels_[i] > 0) {
                if (alg_hnsw_merge->linkLists_[i] != nullptr) {
                    free(alg_hnsw_merge->linkLists_[i]);
                }
                alg_hnsw_merge->linkLists_[i] = (char*)malloc(alg_hnsw_merge->size_links_per_element_ * alg_hnsw_merge->element_levels_[i]);
                if (alg_hnsw_merge->linkLists_[i] == nullptr) {
        ...
        for (size_t i = 0; i < total_elements; ++i) {
            ...
            mergegraph::linklistsizeint* dest_ll_0 = alg_hnsw_merge->get_linklist0(i);
            alg_hnsw_merge->setListCount(dest_ll_0, neighbor_count_0);
            mergegraph::tableint* dest_neighbors_0 = (mergegraph::tableint*)(dest_ll_0 + 1);
            ...
            int element_level = alg_hnsw_merge->element_levels_[i];
            for (int level = 1; level <= element_level; ++level) {
                mergegraph::linklistsizeint* source_ll = source_graph->get_linklist(old_id, level);
                size_t neighbor_count = source_graph->getListCount(source_ll);
                mergegraph::tableint* source_neighbors = (mergegraph::tableint*)(source_ll + 1);

                mergegraph::linklistsizeint* dest_ll = alg_hnsw_merge->get_linklist(i, level);
                alg_hnsw_merge->setListCount(dest_ll, neighbor_count);
                mergegraph::tableint* dest_neighbors = (mergegraph::tableint*)(dest_ll + 1);
    ...
    void hnsw_merge_Skeleton_bridged_Local_Repair(mergegraph::HierarchicalNSW<float>* alg_hnsw_merge,mergegraph::HierarchicalNSW<float>* first_half,mergegraph::HierarchicalNSW<float>* second_half,std::vector<mergegraph::labeltype> & delete_ids,int random_part=10){
        ...
        copy_meta_data(alg_hnsw_merge,first_half,second_half);
        ...
        size_t search_para = alg_hnsw_merge->M_;
        first_half->setEf(search_para);
        second_half->setEf(search_para);
        alg_hnsw_merge->setEf(search_para);
        ...
        alg_hnsw_merge->enterpoint_node_ = second_half->enterpoint_node_ + first_half_size;
        #pragma omp parallel for
        for(size_t k = 0 ; k < first_half_size;++k){
            ...
            alg_hnsw_merge->get_one_hop_neigh(skeleton,layer,two_hop_neighs);
            for(auto && neigh:two_hop_neighs){
                visited_set.insert(neigh);
                if(neigh!=skeleton&&!alg_hnsw_merge->isMarkedDeleted(neigh))
                    visited_pair_1.emplace_back(neigh,alg_hnsw_merge->fstdistfunc_(alg_hnsw_merge->getDataByInternalId(skeleton),alg_hnsw_merge->getDataByInternalId(neigh),alg_hnsw_merge->dist_func_param_));
            }

            for(auto && node:visited_pair_1){
                if(top_candidates.size()<alg_hnsw_merge->maxM0_){
            ...
            alg_hnsw_merge->searchKnn_with_visited_lists(alg_hnsw_merge->getDataByInternalId(skeleton),search_para,visited_pair_2);

            for(auto && node:visited_pair_2){
                if(alg_hnsw_merge->label_lookup_[node.first]==skeleton||visited_set.find(alg_hnsw_merge->label_lookup_[node.first])!=visited_set.end())continue;
                if(top_candidates.size()<alg_hnsw_merge->maxM0_){
                    top_candidates.emplace(node.second,alg_hnsw_merge->label_lookup_[node.first]);
                }
                else if(node.second < top_candidates.top().first){
                    top_candidates.pop();
                    top_candidates.emplace(node.second,alg_hnsw_merge->label_lookup_[node.first]);
            ...
            while(top_candidates.size()>alg_hnsw_merge->maxM0_)top_candidates.pop();
            alg_hnsw_merge->mutuallyConnectNewElement_(alg_hnsw_merge->getDataByInternalId(skeleton),skeleton,top_candidates,0,true,true);
        }

        alg_hnsw_merge->enterpoint_node_ = first_half->enterpoint_node_; 

        #pragma omp parallel for
        for(size_t k = first_half_size; k < alg_hnsw_merge->cur_element_count;++k){
            if(random()%100>=random_part)continue;
            ...
            for(auto && node:visited_pair_1){
                if(top_candidates.size()<alg_hnsw_merge->ef_construction_){
                    top_candidates.emplace(node.second,node.first);
                }
                else if(node.second < top_candidates.top().first){
                    top_candidates.pop();
        ...
        alg_hnsw_merge->enterpoint_node_ = alg_hnsw_merge->get_navi_point();

        alg_hnsw_merge->maxlevel_ = 0 ;
        auto end = std::chrono::high_resolution_clock::now();
        std::cout<<"hnsw merge time: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1e9<<" s"<<std::endl;
    }


    void hnsw_merge_Skeleton_bridged_Local_Repair(mergegraph::HierarchicalNSW<float>* alg_hnsw_merge,std::vector<mergegraph::HierarchicalNSW<float>*> &source_graph,std::vector<mergegraph::labeltype> & delete_ids){

        std::cout << "Starting HNSW merge..." << std::endl;
        auto start = std::chrono::high_resolution_clock::now();

        copy_meta_data_from_multiple(alg_hnsw_merge,source_graph);
        ...
        std::uniform_int_distribution<mergegraph::tableint> distrib(0, alg_hnsw_merge->cur_element_count - 1);
        for(int i = num_graph ; i < num_graph * 2 ; ++i){

            alg_hnsw_merge->enterpoint_node_ = offests_[(i-1)%num_graph] + source_graph[(i-1)%num_graph]->enterpoint_node_;

            int next_offset = i%num_graph < num_graph-1 ? offests_[(i+1)%num_graph]:(unsigned long)alg_hnsw_merge->cur_element_count;
            #pragma omp parallel for
            for(int k = offests_[i%num_graph] ; k < next_offset ; ++k){
                ...
                while(top_candidates.size()>alg_hnsw_merge->maxM0_)top_candidates.pop();
                alg_hnsw_merge->mutuallyConnectNewElement_(alg_hnsw_merge->getDataByInternalId(skeleton),skeleton,top_candidates,0,true);
    ...
    void hnsw_merge_cross_all_sub_graph(mergegraph::HierarchicalNSW<float>* alg_hnsw_merge,std::vector<mergegraph::HierarchicalNSW<float>*> &source_graph,std::vector<mergegraph::labeltype> & delete_ids){
        ...
        for(int i = 0 ; i < num_graph ; ++i){
            ...
            {
                if(j==i)continue;
                alg_hnsw_merge->enterpoint_node_ = offests_[j] + source_graph[j]->enterpoint_node_;
                ...
                for(int k = offests_[i%num_graph] ; k < next_offset ; ++k){
                    ...
                    source_graph[j]->searchKnn_with_visited_lists(alg_hnsw_merge->getDataByInternalId(skeleton),search_para,visited_pair_2);
    ...
    void set_data(mergegraph::HierarchicalNSW<float>* alg_hnsw,std::vector<float*> vectors,int start , int end){
        alg_hnsw->cur_element_count = end-start;
        int max_l = 0 ;
        for(size_t i = start ; i < end ; ++i){
            alg_hnsw->label_lookup_[i] = (mergegraph::tableint)(i-start);

            memcpy(alg_hnsw->getExternalLabeLp((mergegraph::labeltype)(i-start)), &i, sizeof(mergegraph::labeltype));

            alg_hnsw->element_levels_[i-start] = 0;

            memcpy(alg_hnsw->getDataByInternalId(i-start), vectors[i], alg_hnsw->data_size_);

        }
    }
    void set_data_with_label(mergegraph::HierarchicalNSW<float>* alg_hnsw,std::vector<float*> vectors,std::vector<mergegraph::labeltype>& label){
        alg_hnsw->cur_element_count = label.size();
        int max_l = 0 ;
        for(size_t i = 0 ; i < label.size() ; ++i){
            alg_hnsw->label_lookup_[label[i]] = (mergegraph::tableint)(i);

            memcpy(alg_hnsw->getExternalLabeLp((mergegraph::labeltype)(i)), &label[i], sizeof(mergegraph::labeltype));

            alg_hnsw->element_levels_[i] = 0;

            memcpy(alg_hnsw->getDataByInternalId(i), vectors[label[i]], alg_hnsw->data_size_);

        }
    }
    const int compare_times = 1;
    ...
    inline double calculate_recall(mergegraph::HierarchicalNSW<float>* alg_hnsw,std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth,std::vector<float*> queries,int ef,int k,bool after_delete = false){
        alg_hnsw->setEf(ef);
        int num_queries = queries.size();
        double recall_hnsw_total = 0.0;
        #pragma omp parallel for reduction(+:recall_hnsw_total) \
        schedule(dynamic) num_threads(9999)
        for (int i = 0; i < num_queries; i++) {
            float* query_data = queries[i];
            auto result_hnsw = alg_hnsw->searchKnn(query_data, k);
    ...
    inline double calculate_recall(mergegraph::HierarchicalNSW<float>* alg_hnsw_first_half,mergegraph::HierarchicalNSW<float>* alg_hnsw_second_half,std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth,std::vector<float*> queries,int ef,int k,bool after_delete = false){
        alg_hnsw_first_half->setEf(ef);
        alg_hnsw_second_half->setEf(ef);
        int num_queries = queries.size();
        double recall_hnsw_total = 0.0;
        #pragma omp parallel for reduction(+:recall_hnsw_total) \
        schedule(dynamic) num_threads(9999)
        for (int i = 0; i < num_queries; i++) {
            float* query_data = queries[i];
            auto result_hnsw_first = alg_hnsw_first_half->searchKnn(query_data, k);
            auto result_hnsw_second = alg_hnsw_second_half->searchKnn(query_data, k);
    ...
    inline std::pair<int,double> quick_get_ef(mergegraph::HierarchicalNSW<float>* alg_hnsw,
                        std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth,
                        std::vector<float*> queries, int k, float target_recall,bool after_delete = false) {
        int left = k;
        int right = 100;
        float recall_right = calculate_recall(alg_hnsw, groundtruth, queries, right, k,after_delete);
        float recall_left_ = calculate_recall(alg_hnsw, groundtruth, queries, left, k,after_delete);
        ...
        while (recall_right < target_recall && right < 8000) {
            left = right;
            right *= 2;
            recall_right = calculate_recall(alg_hnsw, groundtruth, queries, right, k,after_delete);
        }
        ...
        while (right - left > 1) {
            ...
            float recall_mid = calculate_recall(alg_hnsw, groundtruth, queries, mid, k,after_delete);
        ...
        float recall_left = calculate_recall(alg_hnsw, groundtruth, queries, left, k,after_delete);
        float recall_right_final = calculate_recall(alg_hnsw, groundtruth, queries, right, k,after_delete);
    ...
    inline std::pair<int,double> quick_get_ef(mergegraph::HierarchicalNSW<float>* alg_hnsw_first_half,mergegraph::HierarchicalNSW<float>* alg_hnsw_second_half,
                        std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth,
                        std::vector<float*> queries, int k, float target_recall,bool after_delete = false) {
        int left = k;
        int right = 100;
        float recall_right = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, right, k,after_delete);
        float recall_left_ = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, left, k,after_delete);
        ...
        while (recall_right < target_recall && right < 8000) {
            left = right;
            right *= 2;
            recall_right = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, right, k,after_delete);
        }
        if (right >= 8000 && recall_right < target_recall) {
            return std::make_pair(1,0.0);
        }
        ...
        while (right - left > 1) {
            int mid = (left + right) / 2;
            float recall_mid = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, mid, k,after_delete);
            if (recall_mid >= target_recall)
                right = mid;
            else
                left = mid;
        ...
        float recall_left = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, left, k,after_delete);
        float recall_right_final = calculate_recall(alg_hnsw_first_half,alg_hnsw_second_half, groundtruth, queries, right, k,after_delete);
        if (std::fabs(recall_left - target_recall) <= std::fabs(recall_right_final - target_recall))
            return std::make_pair(left, recall_left);
        else
            return std::make_pair(right, recall_right_final);
    ...
    inline std::pair<double, double> calculate_qps(mergegraph::HierarchicalNSW<float>* alg_hnsw,std::vector<float*> queries,int ef,bool after_delete = false){
        alg_hnsw->setEf(ef);
        int num_queries = queries.size();
        ...
        for (int j = 0;j< times; j++){
            ...
            for (int i = 0; i < num_queries; i++) {
                result_hnsw[i] = alg_hnsw->searchKnn((void *)(q+i*dim*sizeof(float)), ef);
            }
    ...
    inline std::pair<double, double> calculate_qps(mergegraph::HierarchicalNSW<float>* alg_hnsw_first_half,mergegraph::HierarchicalNSW<float>* alg_hnsw_second_half,std::vector<float*> queries,int ef,bool after_delete = false){
        alg_hnsw_first_half->setEf(ef);
        alg_hnsw_second_half->setEf(ef);
        int num_queries = queries.size();
        size_t dim = *(size_t *)alg_hnsw_first_half->dist_func_param_;
        char * q = (char *)malloc(num_queries*sizeof(float)* dim);
        for(int i = 0 ; i < num_queries;++i){
            memcpy(q+dim*sizeof(float)*i,queries[i],dim*sizeof(float));
        }
        ...
        for (int j = 0;j< times; j++){
            auto temp1 = std::chrono::high_resolution_clock::now();
            for (int i = 0; i < num_queries; i++) {
                result_hnsw_first[i] = alg_hnsw_first_half->searchKnn((void *)(q+i*dim*sizeof(float)), ef);
                result_hnsw_second[i] = alg_hnsw_second_half->searchKnn((void *)(q+i*dim*sizeof(float)), ef);
            }
            auto temp2 = std::chrono::high_resolution_clock::now();
            hnsw_search_time_st += std::chrono::duration_cast<std::chrono::microseconds>(temp2 - temp1).count();
            if(j==0){
    ...
    inline void draw_recall_qps_graph(mergegraph::HierarchicalNSW<float> *alg_hnsw,std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth, std::vector<float *> queries, int k,std::string name){
        float start = 0.9;
        for(int i = 0 ; i < 10 ; i ++){
            auto ef = quick_get_ef(alg_hnsw,groundtruth,queries,k,start);
            auto time = calculate_qps(alg_hnsw, queries, ef.first);
            if(ef.first!=0){
    ...
    inline void cal_best_recall(mergegraph::HierarchicalNSW<float> *alg_hnsw,std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth, std::vector<float *> queries, int k,std::string name){
        float start = 0.9;
        for(int i = 0 ; i < 10 ; i ++){
            auto ef = quick_get_ef(alg_hnsw,groundtruth,queries,k,start);
            if(ef.first!=0){
                std::cout<<name<<std::endl;
                std::cout<<"|"<<ef.first<<"|"<<ef.second<<"|"<<std::endl;
            }
            start += 0.01;
        }
    }

    inline void draw_recall_qps_graph(mergegraph::HierarchicalNSW<float> *alg_hnsw_firsrt_half,mergegraph::HierarchicalNSW<float> *alg_hnsw_second_half,std::vector<std::unordered_set<mergegraph::labeltype>> groundtruth, std::vector<float *> queries, int k,std::string name){
        float start = 0.9;
        for(int i = 0 ; i < 10 ; i ++){
            auto ef = quick_get_ef(alg_hnsw_firsrt_half,alg_hnsw_second_half,groundtruth,queries,k,start);
            auto time = calculate_qps(alg_hnsw_firsrt_half,alg_hnsw_second_half, queries, ef.first);
            if(ef.first!=0){
                std::cout<<name<<std::endl;
