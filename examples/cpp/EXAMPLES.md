...
int main() {
    ...
    mergegraph::HierarchicalNSW<float>* alg_hnsw = new mergegraph::HierarchicalNSW<float>(&space, max_elements, M, ef_construction);
    ...
    float correct = 0;
    for (int i = 0; i < max_elements; i++) {
        std::priority_queue<std::pair<float, mergegraph::labeltype>> result = alg_hnsw->searchKnn(data + i * dim, 1);
        mergegraph::labeltype label = result.top().second;
        if (label == i) correct++;
    }
    ...
    alg_hnsw = new mergegraph::HierarchicalNSW<float>(&space, hnsw_path);
    correct = 0;
    for (int i = 0; i < max_elements; i++) {
        std::priority_queue<std::pair<float, mergegraph::labeltype>> result = alg_hnsw->searchKnn(data + i * dim, 1);
        mergegraph::labeltype label = result.top().second;
...
class PickDivisibleIds: public mergegraph::BaseFilterFunctor {
unsigned int divisor = 1;
 public:
    PickDivisibleIds(unsigned int divisor): divisor(divisor) {
        assert(divisor != 0);
    }
    bool operator()(mergegraph::labeltype label_id) {
...
int main() {
    ...
    int k = 10;
    for (int i = 0; i < max_elements; i++) {
        std::vector<std::pair<float, mergegraph::labeltype>> result = alg_hnsw->searchKnnCloserFirst(data + i * dim, k, &pickIdsDivisibleByTwo);
...
int main() {
    ...
    // Initing index
    mergegraph::L2Space space(dim);
    mergegraph::HierarchicalNSW<float>* alg_hnsw = new mergegraph::HierarchicalNSW<float>(&space, max_elements, M, ef_construction, 100, true);
